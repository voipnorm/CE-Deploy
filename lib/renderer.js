...
document.getElementById('deploy').addEventListener('click', (event) => {
    ...
    log.info("Data: "+user+ pwd+macro+macroName+inRoom+endpointCSV+ wallpaper);
    ipcRenderer.send('form-submission', [user, pwd,macro,macroName,inRoom,endpointCSV, wallpaper]);
})
