...
ipcMain.on('form-submission', function (event, data) {
    (async  _ => {
        try{
            ...
            log.info(data);
            const [username, password,macrosPath,macroName,macroActivate,inRoomPath,csvPath, wallpaperPath] = data;
            ...
            const deployFiles = await deploy.deployMacros(username, password, macrosPath, macroName,macroActivate, inRoomPath, wallpaperPath);
