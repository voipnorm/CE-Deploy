...
ipcMain.on('form-submission', (event, data) => {
    (async  _ => {
        try{
            ...
            log.info(data);
            const [username, password,macrosPath,macroName,macroActivate,inRoomPath,csvPath, wallpaperPath, bBPath,bLPath] = data;
...
ipcMain.on('check-id', (event, data) => {
    (async _ => {
        try{
            const [username, password,csvPath] = data;
            ...
            const checkCreds = await deploy.checkCredentials(username,password,endpointIP);
