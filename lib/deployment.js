        ...
        return new Promise((resolve, reject) => {
            (async () => {
                try {
                    ...
                    let  macroXml, inRoomXml, brandingXML, logDir, signageXml, webexTouchXml, oldNewPhraseXml,disableAssistXml;
                    ...
                    if(oldNewPhrase!=null){
                        oldNewPhraseXml = await xml.adminPPhrase(oldNewPhrase)
                    }
                    if(disableAssist === 'true'){
                        disableAssistXml = await xml.disableAssistant();
                    }
                    ...
                    var promises = endpointArray.map(async endpointIp => {
                        try {
                            ...
                            if(webexTouch === 'true'){
                                log.info("Enabling Touch 10")
                                await tp.postXML(webexTouchXml, security);
                            }
                            if(oldNewPhrase != null){
                                log.info("Updating phase for "+username);
                                await tp.postXML(oldNewPhraseXml, security);
                            }
                            if(disableAssist === 'true'){
                                log.info("Disabling Webex Assistant");
                                await tp.postXML(disableAssistXml, security);
                            }
